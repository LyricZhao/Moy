...
struct TransposeOpLowering: public ConversionPattern {
    explicit TransposeOpLowering(MLIRContext *ctx):
        ConversionPattern(moy::TransposeOp::getOperationName(), 1, ctx) {}
    ...
    matchAndRewrite(Operation *op, llvm::ArrayRef<Value> operands,
        ...
        lowerOpToLoops(op, operands, rewriter,
            ...
            // TransposeOp. This allows for using the nice named accessors
            // that are generated by the ODS.
            moy::TransposeOpAdaptor transposeOpAdaptor(memRefOperands);
            Value input = transposeOpAdaptor.input();
...
struct MoyToAffineLoweringPass: public PassWrapper<MoyToAffineLoweringPass, FunctionPass> {
...
void MoyToAffineLoweringPass::runOnFunction() {
    ...
    // Verify that the given main has not inputs and results.
    ...
    // We also define the Moy dialect as Illegal so that the conversion will fail
    // if any of these operations are not converted. Given that we actually want a
    // partial lowering, we explicitly mark the Moy operations that don't want to lower,
    // `moy.print`, as `legal`
    target.addIllegalDialect<moy::MoyDialect>();
    target.addLegalOp<moy::PrintOp>();

    // Now that the conversion target has been defined, we just need to provide
    // the set of patterns that will lower the Moy operations.
    ...
    // conversion. The conversion will signal failure if any of out illegal
...
std::unique_ptr<Pass> mlir::moy::createLowerToAffinePass() {
    return std::make_unique<MoyToAffineLoweringPass>();
}
